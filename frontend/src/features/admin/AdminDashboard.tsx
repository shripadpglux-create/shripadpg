...
export function AdminDashboard({ tab = "Dashboard", isStaffMode = false }: { tab?: string; isStaffMode?: boolean }) {
  ...
  const handleTabClick = (tabName: string) => {
    ...
    if (isStaffMode) {
      navigate({ to: `/staff/${normalized.toLowerCase()}` as any });
    } else {
      navigate({ to: `/admin/${normalized.toLowerCase()}` as any });
    }
  };
  ...
  return (
    <div className="flex min-h-screen bg-slate-100/60 font-sans text-slate-800 selection:bg-brand-green selection:text-white relative overflow-x-hidden">
      ...
      {/* Main Content Area */}
      <div className={`flex flex-1 flex-col transition-all duration-300 min-w-0 pb-24 lg:pb-8 ${isSidebarCollapsed ? "lg:pl-0" : "lg:pl-72"}`}>
          ...
          <header className="mx-auto max-w-7xl rounded-full border border-slate-200/80 bg-white/95 shadow-lg shadow-slate-200/50 backdrop-blur-xl px-2.5 sm:px-6 py-2 sm:py-2.5 flex items-center justify-between gap-2 sm:gap-3 transition-all">
            ...
            <div className="flex items-center gap-1.5 sm:gap-3 shrink-0 flex-nowrap">
              ...
              <button
                ...
                className="hidden lg:flex h-8 w-8 sm:h-9 sm:w-9 shrink-0 items-center justify-center rounded-full border border-slate-200/80 bg-slate-50 text-slate-600 hover:bg-slate-100 shadow-2xs cursor-pointer active:scale-95"
...
function doPost(e) {
  ...
  return ContentService.createTextOutput(JSON.stringify({ result: "success", driveUrl: docUrl })).setMimeType(ContentService.MimeType.JSON);
      ...
      {/* ═══════════════════════════════════════════════════════════════════ */}
      {/* MOBILE PORTAL: FAB, MORE DRAWER, BOTTOM NAVIGATION DOCK          */}
      {/* ═══════════════════════════════════════════════════════════════════ */}
        ...
        <>
          ...
          {isFabMenuOpen && (
            <div
              ...
              className="fixed inset-0 z-[60] bg-slate-900/40 backdrop-blur-[2px] transition-opacity lg:hidden"
            />
          )}

          {/* FAB Expandable Action Menu */}
          <div className={`fixed right-4 z-[61] lg:hidden transition-all duration-300 ease-out ${
          ...
          {isMoreDrawerOpen && (
            <>
              ...
              <div
                ...
                className="fixed inset-0 z-[70] bg-slate-900/60 backdrop-blur-[2px] animate-in fade-in duration-200 lg:hidden cursor-pointer"
              />

              {/* More Drawer Sheet */}
              <div className="fixed bottom-0 left-0 right-0 z-[71] lg:hidden animate-in slide-in-from-bottom duration-250 ease-out">
          ...
          <nav
            ...
            className="fixed bottom-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-xl border-t border-slate-200/90 px-2 py-1.5 flex lg:hidden shadow-2xl items-center justify-around"
